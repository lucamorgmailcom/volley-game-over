...
class Player extends CourtObject {
    constructor(playerConfig, courtConfig, svgConfig) {
        ...
        this.img = playerConfig.img ? playerConfig.img : null
    ...
    async draw(time) {
        if (!this.drawn) {
            ...
            let radius = 54 * this.svg.scale;
            if (this.img != null) {
                this.circle = this.svg.snapRoot.image(this.img, 0-radius, 0-radius, radius*2, radius*2)
                this.courtObject = this.svg.snapRoot.group(this.circle)
                this.courtObject.attr({
                    stroke: this.colours.playerOutlineColour,
                    strokeWidth: 5 * this.svg.scale,

                })
            } else {
                this.circle = this.svg.snapRoot.circle(0, 0, radius)
